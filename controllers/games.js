...
const performGameValidation = (request, response, next) => {
  ...
  if (
    ...
    response.redirect(
      `?Name=${errors["Name"]}&Size=${errors["Size"]}&Duration=${errors["Duration"]}`,
    );
  }
};
...
const getAllGames = (request, response, next) => {
  ...
  pool.query(
    `SELECT * FROM game ORDER BY ${sortBy} ${order}`,
    (error, result) => {
      if (error) {
        throw error;
      }
...
const editGame = (request, response, next) => {
  ...
  pool.query(
    ...
    (error, result) => {
      ...
      response.redirect(
        "/games/?messageToShow=Edited " + request.body.name + "#game" + id,
      );
    },
  );
};
